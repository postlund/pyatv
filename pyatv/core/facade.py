...
from pyatv.const import FeatureName, FeatureState, InputAction, Protocol, HidEventMode
...
class FacadeTouchGestures(Relayer, interface.TouchGestures):
    ...
    async def touch(self, start_x: int, start_y: int, end_x: int, end_y: int, duration_ms: int) -> None:
        ...
        return await self.relay("touch")(start_x=start_x, start_y=start_y,
    ...
    async def touch_event(self, x: int, y: int, mode: HidEventMode) -> None:
        ...
        return await self.relay("touch_event")(x=x, y=y, mode=mode)
