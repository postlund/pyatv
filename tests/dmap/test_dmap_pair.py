...
class DmapPairFunctionalTest(AioHTTPTestCase):
    def setUp(self):
        ...
        self.service = DmapService("dmap_id", PAIRING_GUID, port=self.server.port)
    ...
    async def get_application(self, loop=None):
        self.fake_atv = FakeDmapService(FakeDmapState(HSGID, PAIRING_GUID, SESSION_ID))
    ...
    async def initiate_pairing(self, name=REMOTE_NAME, pairing_guid=PAIRING_GUID):
        ...
        self.pairing = await pyatv.pair(self.conf, Protocol.DMAP, self.loop, **options)
